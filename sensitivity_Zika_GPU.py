...
def find_Adj(edge_index_dict):
    ...
    # Self loop
    A_PPI = (A_PPI + torch.eye(x_prot['num_nodes']).to(device)).clamp(max=1)
    A_DDI = (A_DDI + torch.eye(x_drug['num_nodes']).to(device)).clamp(max=1)
...
pick_threshold = 0.025 
pick_threshold = 0.005 
# len(node_degree_select_indices)
node_degree_select_indices = node_degree_select[1][0: int(len(node_indices)*pick_threshold)]

node_degree_select_indices = torch.sort(node_degree_select_indices, descending = False)[0]
...
with open("time_collapse.txt", "w") as file:
    for i in range(test_interation):
        ...
        for j in range(test_interation):
            ...
            term12 = ( I_n * a_ij) @  partial_log_term_partial_A_bar(H_li, H_lj, A_bar, A_hat_ij, mask, i, j)
...
with open("output.txt", "w") as file:
  file.write(str(partial.to_dense().tolist()))

"""-----

------**Find and remove less sensitive nodes**
"""